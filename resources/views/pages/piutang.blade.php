        ...
        <div class="intro-y col-span-12">
            <div class="grid grid-cols-2 gap-4">
                <div class="box p-5 bg-primary/5">
                    <div class="text-slate-500 text-sm">Total Utang</div>
        ...
        <div class="intro-y col-span-12 overflow-auto">
            <table class="table table-report -mt-2">
                ...
                <tbody>
                    ...
                    <tr class="intro-x">
                        <td class="!py-4">
                            <a href="{{ route('piutang.show', $p->id_produksi) }}" class="underline decoration-dotted font-medium text-primary">
                                #{{ $p->id_produksi }}
                            </a>
                        </td>
                        <td>
                        ...
                        <td>{{ $p->produksi->tanggal }}</td>
                        <td class="text-right">Rp {{ number_format($p->total_tagihan, 0, ',', '.') }}</td>
                        <td class="text-right text-success">Rp {{ number_format($p->total_terbayar, 0, ',', '.') }}</td>
                        <td class="text-right font-medium {{ $p->sisa_tagihan > 0 ? 'text-warning' : 'text-success' }}">
...
<style media="print">
    ...
    .intro-y, .intro-y * { visibility: visible; }
    .intro-y { position: absolute; left: 0; top: 0; width: 100%; }
    .btn, .dropdown, nav, .side-menu, form, select { display: none !important; }
    table { width: 100%; border-collapse: collapse; }
    th, td { padding: 8px; border: 1px solid #ddd; }
    .text-primary, .text-success, .text-warning { color: #000 !important; }
</style>
@endsection
